...
for k=1 : size(factors)
    factor = factors(k,:);
    negfac = factor(1);
    fstfac = factor(2);

    outf = sprintf('const_models_2sigma_hull/co14_const_%.4f_%.4f.csv', negfac, fstfac);
    fprintf('%3d %.4f %.4f %s\n', k, negfac, fstfac, outf);
    ...
    % P_mu_neg_14CO = Balco_mu_neg.*0.068; %this applies the F*Omega tuning factor discussed in intro
    % P_mu_fast_14CO = Balco_mu_fast.*0.062;    
    ...
    %------------------------------------------
    % P_change = 1.5;  %values above one mean production rates increase with time
    % P_change_step = (P_change - 1)/double(no_t_pts - 1);
    % 
    % for i=2 : no_t_pts  % i is serving as the counter for time
    %     dP = double(1+ P_change_step*double(i-1));
    %     if i > (no_t_pts - 1)
    %         disp(dP);
    %     end
    ...
    %------------------------------------------
    % P_change = 1 % magnitude (factor) of transient change in production rate
