...
for ifac=1 : 1
    ...
    for k=1 : no_t_pts
        ...
        outf = sprintf('delta_fast_models/co14_delta_%.4f_%.4f_%.4f.csv', k, negfac, fstfac);
        fprintf('%3d %.4f %.4f %3d %.4f %s\n', ifac, negfac, fstfac, k, k, outf);
        ...
        % Production rate matrices (in time and depth):
        Pmn = zeros(no_t_pts, no_depths); %first index is the time dimension, 2nd index is the depth dimension
        Pmf = zeros(no_t_pts, no_depths);
        ...
        for i=1 : no_depths
            Pmn(k,i) = P_mu_neg_14CO(i);
            Pmf(k,i) = P_mu_fast_14CO(i);
        end
