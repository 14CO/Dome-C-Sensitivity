...
% for ifac=1 : 1
    ...
    for k=1 : length(P_changes)
        ...
        outf = sprintf('linear_models_past/co14_linear_%.4f_%.4f_%.4f.csv', P_changes(k), negfac, fstfac);
        ...
        %------------------------------------------
        % P_change = 1.0; % magnitude (factor) of transient change in production rate
